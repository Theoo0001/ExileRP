...
ESX.ResetSlots = function()	
	ESX.PlayerData.slots = {}
	...
	TriggerServerEvent('esx:updateSlots', ESX.PlayerData.slots)
end

ESX.SetSlot = function(value, slot)
	
	--remove old slot
	if ESX.PlayerData.slots[value] then
		SendNUIMessage({
			action = 'updateSlot',
			slot = ESX.PlayerData.slots[value],
			name = nil
		})				

		ESX.PlayerData.slots[value] = nil
	end
	...
	--set slot
	ESX.PlayerData.slots[value] = slot
...
ESX.GetMisiaczekSlots = function()
	return ESX.PlayerData.slots
end
...
AddEventHandler('pNotify:SendNotification', function(data)
	if data.text then
		TriggerEvent("FeedM:showNotification", data.text)
	end
end)
...
ESX.Game.Utils.RenderHeadshotInternal = function(coords, skin, clbk)
	local ped = Citizen.InvokeNative(0xD49F9B0955C367DE, 3, (skin['sex'] > 1 and (exports['skinchanger']:GetCustomPed(skin['sex']) ~= false or `mp_m_freemode_01`) or (skin['sex'] == 0 and `mp_m_freemode_01` or `mp_f_freemode_01`)), coords.x, coords.y, coords.z - 50.0, 0.0, false, false)
...
ESX.Game.GetClosestObject2 = function(filter, coords)
	...
	for i=1, #objects, 1 do
		...
		if foundObject then
			...
			local distance     = #(objectCoords - vec3(coords.x, coords.y, coords.z))
...
ESX.Game.GetVehicleInDirection = function(v, h, z)
	v = v or 0.0
	h = h or 5.0
	z = z or -0.95
	...
	local inDirection  = GetOffsetFromEntityInWorldCoords(playerPed, v, h, z)
...
ESX.ShowInventory = function()
	...
	for k,v in ipairs(ESX.PlayerData.inventory) do
		if v.count > 0 then
			...
			if v.type == 'item' then
				table.insert(elements, {
					label = (ESX.PlayerData.slots[v.name] and (v.count > 1 and ('%s/%s %s <span style="color:#7cfc00; font-size: 15.0px; float:right;">%s</span>'):format(v.count, v.limit, v.label, ESX.PlayerData.slots[v.name]) or ('%s <span style="color:#7cfc00; font-size: 15.0px; float:right;">%s</span>'):format(v.label, ESX.PlayerData.slots[v.name])) or (v.count > 1 and ('%s/%s %s'):format(v.count, v.limit, v.label) or v.label)),
					count = v.count,
					type = 'item_standard',
					value = v.name,
					usable = v.usable,
			...
			elseif v.type == 'weapon' then					
				for _,d in ipairs(Config.Weapons) do
					if v.data.name == d.name then 
						if d.take_ammo == nil then
							d.take_ammo = false
						end

						if ESX.PlayerData.slots[v.name] then					
							if d.take_ammo then
								label = (v.data.serial_number == false and ('%s [%s] <span style="color:#7cfc00; font-size: 15.0px; float:right;">%s</span>'):format(v.label, v.data.ammo, ESX.PlayerData.slots[v.name]) or ('%s [%s] [%s] <span style="color:#7cfc00; font-size: 15.0px; float:right;">%s</span>'):format(v.label, v.data.ammo, v.data.serial_number, ESX.PlayerData.slots[v.name]))
							else
								label = (v.data.serial_number == false and ('%s <span style="color:#7cfc00; font-size: 15.0px; float:right;">%s</span>'):format(v.label, ESX.PlayerData.slots[v.name]) or ('%s [%s] <span style="color:#7cfc00; font-size: 15.0px; float:right;">%s</span>'):format(v.label, v.data.serial_number, ESX.PlayerData.slots[v.name]))
							end
						else
							if d.take_ammo then
								label = (v.data.serial_number == false and ('%s [%s]'):format(v.label, v.data.ammo) or ('%s [%s] [%s]'):format(v.label, v.data.ammo, v.data.serial_number))
							else
								label = (v.data.serial_number == false and ('%s'):format(v.label) or ('%s [%s]'):format(v.label, v.data.serial_number))
							end
						end	
			...
			elseif v.type == 'sim' then
				table.insert(elements, {
					label = (ESX.PlayerData.slots[v.name] and ('%s <span style="color:#7cfc00; font-size: 15.0px; float:right;">%s</span>'):format(v.label, ESX.PlayerData.slots[v.name]) or v.label),
					count = 1,
					type = 'item_sim',
					value = v.name,
					usable = true,
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory', {
		...
		if data.current.canRemove then
			...
			if not IsPedInAnyPoliceVehicle(playerPed, false) then
		...
		if ESX.PlayerData.slots[data.current.value] then
		...
		if data.current.type == 'item_weapon' then
			if data.current.canGiveAmmo then
				table.insert(elements, {label = 'Amunicja: '..tostring(data.current.data.ammo), action = nil})
			end
			
			table.insert(elements, {label = 'Numer seryjny: '..(data.current.data.serial_number ~= false and tostring(data.current.data.serial_number) or 'Brak'), action = nil})
			if data.current.canGiveAmmo and data.current.data.ammo ~= nil then
				if data.current.data.ammo > 32 then
					table.insert(elements, {label = 'Wyciągnij magazynek', action = 'give_ammo', type = data.current.type, value = data.current.value})
				end
			end
		end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory_item', {
			...
			if data1.current.action == 'unbind' then				
				if ESX.PlayerData.slots[data.current.value] then				
					ESX.SetSlot(data.current.value, nil)
				end
						...
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'give_item_to', {
							...
							if playersNearby[selectedPlayer] then
								if data.current.count > 1 then 
									...
									menu2.close()
									menu1.close()
									TriggerServerEvent('esx:gitestveInventoryItem', selectedPlayerId, type, item, 1)
								end
							else
			...
			elseif data1.current.action == 'give_ammo' then
				if type == 'item_weapon' then	
					local takeammo = false
					
					local newammo = 0
					for k,v in ipairs(ESX.PlayerData.inventory) do	
						if v.name == data.current.value then														
							if v.data.ammo >= 32 then
								v.data.ammo = v.data.ammo - 32
								
								Citizen.InvokeNative(0x14E56BC5B5DB6A19, playerPed, GetHashKey(v.data.name), v.data.ammo)
					...
					if not takeammo then
						ESX.ShowNotification('~r~Nie możesz wyciągnąć magazynek')
					else						
						TriggerServerEvent('es_extneded:giveclip', data.current.canGiveAmmo)
						ESX.ShowNotification('~s~Udało ci się wyciągnąć magazynek z ~o~'..data1.current.label)
						TriggerServerEvent('esx:updateItem', data.current.value, 'ammo', newammo)
					end
				end			
			end
