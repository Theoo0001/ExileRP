...
RegisterNetEvent("openRM")
AddEventHandler("openRM", function()
...
RegisterNUICallback('loadRaces', function()
	SetNuiFocus(false, false)
...
RegisterNUICallback('createRace', function()
    ...
    ESX.ShowNotification("Otwórz mapę i zacznij zaznaczać trasę wyścigu")
end)

RegisterNUICallback('saveRace', function(data)
	SetNuiFocus(false, false)
    if data.name ~= nil and #recordedCheckpoints > 0 then
        -- Send event to server to save checkpoints
        TriggerServerEvent('StreetRaces:saveRace_sv', data.name, recordedCheckpoints)
    end
end)

RegisterNUICallback('startRace', function(data)
	SetNuiFocus(false, false)
    local amount = tonumber(data.kwota)
    if amount then
        -- Get optional start delay argument and starting coordinates
        local startDelay = tonumber(data.czas)
        ...
        if #recordedCheckpoints > 0 then
            print("Wystartowałeś wyścig. Zacznie się za ~g~"..(startDelay/1000).."~s~ a kwota dołączenia to ~b~"..amount.."~s~. Aby wziąć w nim udział, wsiądź do samochodu!")
            ESX.ShowNotification("Wystartowałeś wyścig. Pakuj się do fury!")
...
RegisterNetEvent("openMenuListRace")
AddEventHandler("openMenuListRace", function(lista)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'test',
    {
        ...
        TriggerServerEvent('StreetRaces:loadRace_sv', data.current.label)
        menu.close()
...
CreateThread(function()
	for k,v in pairs(config_stations) do
		...
		AddTextComponentSubstringPlayerName('Wyścigi samochodowe')
...
CreateThread(function()
	while true do
        ...
        for i=1, #config_stations, 1 do
            local distance = #(playerCoords - config_stations[i])

            if distance < 20 then
                ESX.DrawMarker(vec3(config_stations[i]))
                letSleep = false

                if distance < 1.0 then
                    isInMarker, currentStation, currentPart, currentPartNum = true, 'test', 'Cloakroom', i
                end
            end
        end
        ...
        if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
            ...
            then
                TriggerEvent('qStreetRaces:hasExitedMarker', LastStation, LastPart, LastPartNum)
                hasExited = true
            end
            ...
            TriggerEvent('qStreetRaces:hasEnteredMarker', currentStation, currentPart, currentPartNum)
        end
...
AddEventHandler('qStreetRaces:hasEnteredMarker', function(station, part, partNum)
	if part == 'Cloakroom' then
		CurrentAction     = 'menu_cloakroom'
		CurrentActionMsg  = 'Naciśnij ~INPUT_PICKUP~ aby otworzyć panel ~n~~g~Exile ~b~Race ~g~ ~BLIP_38~'
		CurrentActionData = {}
	end
end)

AddEventHandler('qStreetRaces:hasExitedMarker', function(station, part, partNum)
...
RegisterCommand("race", function(source, args)
    ...
    elseif args[1] == "cancel" then
        ...
        ESX.ShowNotification("Podaj argument")
        return
    end
end)
        ...
        if IsPedInAnyVehicle(player, false) then
            ...
            elseif raceStatus.state == RACE_STATE_JOINED then
                ...
                elseif count <= config_cl.freezeDuration then
                    ...
                    Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+1.0, ("Wyścig o ~g~$%d~w~ startuje za ~y~%d~w~s"):format(race.amount, math.ceil(count/1000.0)))
                    Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+0.80, "Dołączono")
                end
            -- Player is not in a race
                ...
                for index, race in pairs(races) do
                    ...
                    if proximity < config_cl.joinProximity and currentTime < race.startTime then
                        ...
                        Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+1.0, ("Wyścig o ~g~$%d~w~ startuje za ~y~%d~w~s"):format(race.amount, count))
                        Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+0.80, "Naciśnij [~g~E~w~] aby dołączyć")
