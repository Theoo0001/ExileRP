        ...
        for index, race in pairs(races) do
            ...
            elseif (race.finishTime ~= 0) and (time > race.finishTime + race.finishTimeout) then
                ...
                for _, player in pairs(players) do
                    notifyPlayer(player, "Dyskwalifikacja (czas minął)")
                end
            ...
            if race.finishTime == 0 then
                ...
                for _, pSource in pairs(players) do
                    if pSource == source then
                        local msg = ("Wygrałeś [%02d:%06.3f]"):format(timeMinutes, timeSeconds)
                        notifyPlayer(pSource, msg)
                    elseif config_sv.notifyOfWinner then
                        local msg = ("Wyścig wygrał %s [%02d:%06.3f]"):format(getName(source), timeMinutes, timeSeconds)
                        notifyPlayer(pSource, msg)
                    end
                end
            else
                -- Loser, send notification to only the player
                local msg = ("Przegrałeś [%02d:%06.3f]"):format(timeMinutes, timeSeconds)
                notifyPlayer(source, msg)
            end
...
AddEventHandler("StreetRaces:deleteRace_sv", function(name)
    ...
    if playerRaces[name] ~= nil then
        ...
        -- Send notification to player
        local msg = "Usunięto " .. name
        notifyPlayer(source, msg)
    else
        local msg = "Nieznaleziono wyścigu z tą nazwą " .. name
        notifyPlayer(source, msg)
    end
end)
...
AddEventHandler("StreetRaces:listRaces_sv", function()
    ...
    for name, race in pairs(playerRaces) do
        table.insert(wyscigi, {label = name})
        count = count + 1
    end
    TriggerClientEvent("openMenuListRace", source, wyscigi)
end)
...
RegisterCommand("rr", function(source, args, rawCommand)
    ...
    MySQL.Async.fetchAll('SELECT firstname, lastname, dateofbirth, height FROM users WHERE identifier = @identifier', {
