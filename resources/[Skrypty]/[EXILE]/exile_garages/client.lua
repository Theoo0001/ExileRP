...
RegisterNUICallback('odholuj_all', function(data, cb)
		...
		ESX.TriggerServerCallback('flux_garages:checkMoney', function(hasMoney)
			if hasMoney == 1 then
				...
				exports['ExileRP']:DrawProcent(100, function()
					TriggerServerEvent('exile_garages:findVehicleAll', data.plate)
					IsImpounding = false
				end)
				IsImpounding = false
			else
...
function OpenStatusCheckMenu()
	local elements = {}
	ESX.TriggerServerCallback('xfsd-vehstatus:getVehicles', function(vehicles)
		local elements = {
