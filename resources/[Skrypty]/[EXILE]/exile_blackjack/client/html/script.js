...
var BlackJackObject = function() {
	...
	this.Win = function() {
		...
		$.post('http://exile_blackjack/WinBet', JSON.stringify({
			bets: obecnyBet
		}));
	...
	this.Tie = function() {
		// Do Something With .bet
		$.post('http://exile_blackjack/TieBet', JSON.stringify({
			bets: obecnyBet
		}));
	...
	this.Message = function(info) {
		// Do Something With .bet
		$.post('http://exile_blackjack/Status', JSON.stringify({
			tekst: info
		}));
		self.myTurn = true;
	}
	...
	this.Lost = function() {
		// Do Something With .bet
		$.post('http://exile_blackjack/LostBet', JSON.stringify({
			bets: obecnyBet
		}));
		self.myTurn = true;
		if (coins < 500) {
			$.post('http://exile_blackjack/escape2', JSON.stringify({}));
		}
		obecnyBet = 500;
	}
...
$(function() {
    window.addEventListener('message', function(event) {
        if (event.data.type == "enableui") {
			...
			if (event.data.enable) {			
				...
				$.post("http://exile_blackjack/card", JSON.stringify({}));
				queActions(104);
				BlackJack.Intro();
				BlackJack.Shuffle();
				...
				$hit.textContent = '[NOWE ROZDANIE]';
				$stay.textContent = '[BET UP] +500';
				$score.textContent = '[BET DOWN] -500';
				$('#ownedPoints').empty().append(0);
				$('#ownedBet').empty().append(500);
			} else {
    ...
    document.onkeyup = function (data) {
        if (data.which == 27) { // Escape key
			$.post('http://exile_blackjack/escape', JSON.stringify({}));
        }
    };
	...
	$hit.addEventListener('click', function () {
		...
		if ($hit.textContent.includes("[NOWE ROZDANIE]") && BlackJack.bet >= 500) {
			$.post("http://exile_blackjack/card", JSON.stringify({}));
			$('#ownedPoints').empty().append(0);
			obecnyBet = BlackJack.bet
			$.post('http://exile_blackjack/StartPartia', JSON.stringify({
				bets: obecnyBet
			}));
			$.post("http://exile_blackjack/card", JSON.stringify({}));
			// NEW GAME
			$hit.textContent = "[DOBIERZ]";
			coins = coins - BlackJack.bet;
			$('#ownedCoins').empty().append(coins);
			$('#ownedBet').empty().append(BlackJack.bet);
			$stay.textContent = "[STOP]";
			$score.textContent = "Punkty dealer'a: 0";
			$.post("http://exile_blackjack/card", JSON.stringify({}));
			...
			var prom = new Promise(function(res, rej) {
				...
				$score.textContent = "Punkty dealer'a: " + BlackJack.getScore(BlackJack.dealerHand.slice(0, 1));
			});
			...
			var prom = new Promise(function(res, rej) {
				...
				if (BlackJack.didBust(BlackJack.hand)) {
					...
					BlackJack.Message('Przegrałeś mając ~y~' + BlackJack.getScore(BlackJack.hand) + ' punktów');
					$hit.textContent = '[NOWE ROZDANIE]';
					$stay.textContent = '[BET UP] +500';
					$score.textContent = '[BET DOWN] -500';
					$('#ownedPoints').empty().append(0);
	...
	$stay.addEventListener('click', function () {
		...
		if ($stay.textContent.includes("[BET UP] +500")) {
			...
			$.post("http://exile_blackjack/bet", JSON.stringify({}));
			...
			$score.textContent = "Punkty dealer'a: " + BlackJack.getScore(BlackJack.dealerHand);
			...
			if (BlackJack.getScore(BlackJack.dealerHand) > BlackJack.getScore(BlackJack.hand)) {
				...
				$score.textContent = '[BET DOWN] -500';
				BlackJack.Lost();
				BlackJack.Message('Przegrałeś ~y~' + BlackJack.getScore(BlackJack.hand) + '~w~ do ~y~' + BlackJack.getScore(BlackJack.dealerHand));
				$('#ownedBet').empty().append(500);
				queActions(0);
				...
				$score.textContent = '[BET DOWN] -500';
				BlackJack.Tie();
				BlackJack.Message('Remis! ~y~' + BlackJack.getScore(BlackJack.hand) + '~w~ do ~y~' + BlackJack.getScore(BlackJack.dealerHand));
				$('#ownedBet').empty().append(500);
				queActions(0);
				...
				$score.textContent = '[BET DOWN] -500';
				BlackJack.Win();
				BlackJack.Message('Wygrałeś ~y~' + BlackJack.getScore(BlackJack.hand) + '~w~ do ~y~' + BlackJack.getScore(BlackJack.dealerHand));
				$('#ownedPoints').empty().append(0);
				$('#ownedBet').empty().append(500);
				queActions(0);
			} else {
