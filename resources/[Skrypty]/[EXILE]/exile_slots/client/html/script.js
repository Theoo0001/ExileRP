...
function endWithWin(x, sound) {
  $.post('http://exile_slots/wygrana', JSON.stringify({
      win: x
    }));
  ...
  if(rolling == 0) {
    if(canDouble == 0 && bet > 0) {
      ...
      if(coins >= bet && coins !== 0) {
        $.post("http://exile_slots/zajebkase", JSON.stringify({
          ile: bet
        }));
        insertCoin(-bet);
        rolling = 1;
...
function togglePacanele(start, banuti) {
  if(start == true) {
    ...
    $.post("http://exile_slots/exitWith", JSON.stringify({
...
$(document).ready(function() {
  ...
  $('#betUp200').on('click', function(){ // WHITE
    pressWHITE();
  })
  
  $('#betUp500').on('click', function(){ // RED
    pressRED();
  })

  $('#betUp1000').on('click', function(){ // YELLOW
    pressYELLOW();
  })

  $('#betUp2000').on('click', function(){ // GREEN
    pressGREEN();
  })

  $('#betUp10000').on('click', function(){ // PURPLE
    pressPURPLE();
  })
