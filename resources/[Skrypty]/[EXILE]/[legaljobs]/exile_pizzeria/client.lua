...
RegisterNetEvent("exile_pizzeria:getrequest")
TriggerServerEvent("exile_pizzeria:request")
AddEventHandler("exile_pizzeria:getrequest", function(a, b, c, d)
	_G.donttry = a
	_G.exile_ca = b
	_G.exile_ca2 = c
	_G.exile_ca3 = d
	local donttouchme = _G.donttry
	local exile_pizzeria = _G.exile_ca
	local exile_pizzeria2 = _G.exile_ca2
	local exile_pizzeria3 = _G.exile_ca3
local ESX, PlayerData, inProgress, pCoords, pizzeriaBlips, CanWork = nil, {}, false, nil, {}, false
...
local Cfg = {
	CollectSeeds = {
		{
			coords = vector3(486.83, -2170.14, 5.91-0.95),--
		},
	},
	TransferingSeeds = {
		{
			coords = vector3(811.3, -759.15, 26.78-0.95),--
		},
	},
	SellCoffee = {
		{
			coords = vector3(798.95, -750.34, 31.26-0.95),--
		},
	},
	Garage = {
		{
			coords = vector3(802.27, -736.84, 27.65-0.95)--
		},
		{
			coords = vector3(491.18, -2156.38, 5.91-0.95)--
		},
	},
	BossActions = {
		{
			coords = vector3(797.34, -767.14, 31.26-0.95)--
		},
	},
	Cloakroom = {
		{
			coords = vector3(812.79, -761.76, 31.26-0.95)--
		},
		{
			coords = vector3(475.67, -2167.78, 5.91-0.95)--
		},
	},
	Clothes = {
		Male = {
...
local Blips = {
	{title="#1 Szatnia", colour=0, id=488, see = true, coords = vector3(812.79, -761.76, 31.26)},
	{title="#2 Zbieranie Ciasta", colour=0, id=488, see = true, coords = vector3(486.83, -2170.14, 5.91)},
	{title="#3 Przeróbka Ciasta", colour=0, id=488, see = true, coords = vector3(811.3, -759.15, 26.78)},
	{title="#4 Punkt dostawy Pizzy", colour=0, id=488, see = false, coords = vector3(798.95, -750.34, 31.26)}
},
...
refreshBlips = function()
	if PlayerData.job ~= nil and PlayerData.job.name == 'pizzeria' then
		for i=1, #Blips, 1 do
			if Blips[i].see then
				...
				table.insert(pizzeriaBlips, blip)
			else
				if PlayerData.job.grade >= 7 then
					...
					table.insert(pizzeriaBlips, blipSell)
				end
			end
		end
	end
...
deleteBlip = function()
	if pizzeriaBlips[1] ~= nil then
		for i=1, #pizzeriaBlips, 1 do
			RemoveBlip(pizzeriaBlips[i])
			pizzeriaBlips[i] = nil
		end
	end
end
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'pizzeria' then
			...
			if isInMarker and not hasAlreadyEnteredMarker then
				...
				TriggerEvent('exile_pizzeria:hasEnteredMarker', currentZone, zoneNumber)
			end
	
			if not isInMarker and hasAlreadyEnteredMarker then
				hasAlreadyEnteredMarker = false
				TriggerEvent('exile_pizzeria:hasExitedMarker', lastZone)
			end
...
CreateThread(function()
	while true do
		...
		Citizen.Wait(10)
		if PlayerData.job and PlayerData.job.name == 'pizzeria' then
...
StopCollect = function()
	TriggerServerEvent(exile_pizzeria3, 'ciasto', donttouchme)
	inProgress = false
	FreezeEntityPosition(PlayerPedId(), false)
	ClearPedTasks(PlayerPedId())
	CurrentAction		= 'collect'
	CurrentActionMsg	= "Naciśnij ~INPUT_CONTEXT~, aby zebrać ~b~Ciasto"
	CurrentActionData	= {}
end
...
StartCollect = function()
	...
	ESX.TriggerServerCallback('gcphone:getItemAmount', function(count)
		if count < 100 then
			...
			inProgress = false
			ESX.ShowNotification('~y~Nie uniesiesz więcej Ciasta')
		end
	end, 'ciasto')
end
...
TransferingSeeds = function()
	...
	for i=1, #inventory, 1 do
		if inventory[i].name == 'ciasto' then
	...
	if kochacpieski == true then
		if count >= 100 then
			...
			ESX.Streaming.RequestAnimDict(lib, function()
				...
				exports["exile_taskbar"]:taskBar(40000, "Trwa przygotowywanie Pizzy", false, true)
				Citizen.InvokeNative(0xAAA34F8A7CB32098, PlayerPedId())
				TriggerServerEvent(exile_pizzeria2, count, donttouchme)
				FreezeEntityPosition(PlayerPedId(), false)
			...
			ESX.ShowNotification('~r~Potrzebujesz 100x ciasto aby rozpocząć przerabianie!')
		end
	else
...
SellCoffee = function()
	...
	local count = 0
	for i=1, #inventory, 1 do
		if inventory[i].name == 'capricciosa' then
			count = inventory[i].count
		end
	end
	if count > 0 then
		...
		ESX.Streaming.RequestAnimDict(lib, function()
			TaskPlayAnim(PlayerPedId(), lib, anim, 8.0, -8.0, -1, 0, 0, false, false, false)
			exports["exile_taskbar"]:taskBar(12000, "Sprzedawanie Pizzy", false, true)
			Citizen.InvokeNative(0xAAA34F8A7CB32098, PlayerPedId())
			TriggerServerEvent(exile_pizzeria, count, donttouchme)
			FreezeEntityPosition(PlayerPedId(), false)
			blokuj = false
		end)
	else
		ESX.ShowNotification('~r~Nie posiadasz przy sobie Pizzy!')
	end
end
...
CarOut = function()
	...
	if IsPedInAnyVehicle(player, false) then
		local carpizzeria = GetVehiclePedIsIn(player, false)
		if IsVehicleModel(carpizzeria, `pizzacar`) then
			ESX.Game.DeleteVehicle(carpizzeria)
			alreadyOut = false
		else
		...
		if ESX.Game.IsSpawnPointClear(GetEntityCoords(PlayerPedId()), 7) then
			ESX.Game.SpawnVehicle('pizzacar', GetEntityCoords(PlayerPedId()), 86.53, function(vehicle)
				platenum = math.random(10, 99)
				SetVehicleNumberPlateText(vehicle, "PIZZ"..platenum)
				TaskWarpPedIntoVehicle(PlayerPedId(), vehicle, -1)
				TriggerServerEvent('exile_pizzeria:insertPlayer', GetVehicleNumberPlateText(vehicle))
			end)
			alreadyOut = true
		else
...
function OpenBossMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pizzeria_boss', {
		title    = "Pizza This",
		align    = 'bottom-right',
		elements = elements
	}, function(data, menu)
		...
		if data.current.value == '1' then
			if PlayerData.job.grade == 7 then
				TriggerEvent('esx_society:openBossMenu', 'pizzeria', function(data, menu)
					menu.close()
			...
			ESX.TriggerServerCallback('ExileRP:getCourses', function(kursy)
				if kursy then
					...
					for i=1, #kursy, 1 do
						...
						ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'pizzeria_courses', elements, function(data, menu)
						end, function(data, menu)
							menu.close()
						end)
                    end
...
RegisterNetEvent('exile_pizzeria:Cancel')
AddEventHandler('exile_pizzeria:Cancel', function()
	local playerPed = PlayerPedId()
	FreezeEntityPosition(playerPed, false)
	ClearPedTasks(playerPed)
end)

AddEventHandler('exile_pizzeria:hasEnteredMarker', function(zone, number)
	...
	if zone == 'TransferingSeeds' then
		...
		CurrentActionMsg	= "Naciśnij ~INPUT_CONTEXT~, aby przerobić ~b~Ciasto"
		CurrentActionData	= {}
	end

	if zone == 'SellCoffee' then
		CurrentAction		= 'sell'
		CurrentActionMsg	= "Naciśnij ~INPUT_CONTEXT~, aby sprzedać ~b~Pizze"
		CurrentActionData	= {}
	end
...
AddEventHandler('exile_pizzeria:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	TriggerServerEvent('exile_pizzeria:stopPickup', zone)
...
OpenMobilepizzeriaActionsMenu = function()
	...
	if IsVehicleModel(vehicle, `pizzacar`) then
		if IsPedInAnyVehicle(playerPed, false) and GetPedInVehicleSeat(vehicle, -1) == playerPed then
			ESX.TriggerServerCallback('exile_pizzeria:checkSiedzacy', function(siedzi)
				if siedzi then
