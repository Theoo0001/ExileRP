...
CreateThread(function()
	...
	Citizen.Wait(5000)
	TriggerServerEvent('esx_mechanicjobdrugidrugi:checkIsDuty')
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mechanic_actions', {
      ...
      elseif data.current.value == 'clean_vehicle' then
		...
		if DoesEntityExist(vehicle) then
			...
			CreateThread(function()
				exports["exile_taskbar"]:taskBar(10000, "Mycie pojazdu", false, true)
...
AddEventHandler('esx_mechanicjobdrugi:hasEnteredMarker', function(zone)
	...
	elseif zone == "VehicleExtras" then
		...
		CurrentActionMsg  = "Naciśnij ~INPUT_CONTEXT~, aby otworzyć menu z dodatkami"
		CurrentActionData = {}
...
CreateThread(function()
	...
	while true do
		Citizen.Wait(3)
		if PlayerData.job.name == "mechanik2" then
...
CreateThread(function()
	...
	while true do
		Citizen.Wait(3)
		if PlayerData.job.name == "mechanik2" or PlayerData.job.name == "offmechanik2" then
				...
				if IsControlJustReleased(0, 38) and PlayerData.job then
					if Config.Zones[PlayerData.job.name] then
						...
						if CurrentAction == 'duty_list' then
							OpenDutyList()
						elseif CurrentAction == 'extras_menu' then
							OpenExtras()	
