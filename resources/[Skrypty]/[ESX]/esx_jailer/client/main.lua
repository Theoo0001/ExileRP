		...
		CreateThread(function()
			...
			while JailTime > 0 do
				...
				if #(GetEntityCoords(sourcePed) - vec3(JailLocation.x, JailLocation.y, JailLocation.z)) > 250.0 then
					...
					if not escapeCooldown then
						--print("SZANSA: "..chance)
						if chance <=5 then
							TriggerServerEvent('esx_jailer:unjailAfterEscape', GetPlayerServerId(PlayerId()))
							TriggerServerEvent('esx_jailer:escapenotifysertver',GetPlayerServerId(PlayerId()))
						else
							escapeCooldown = true
							TriggerServerEvent("esx_jailer:triedToEscape", GetPlayerServerId(PlayerId()))
							SetEntityCoords(sourcePed, JailLocation.x, JailLocation.y, JailLocation.z)
							TriggerEvent('chat:addMessage1', "Sędzia", { 147, 196, 109 }, "Z więzienia nie uciekniesz!", "fas fa-gavel")
						end
					else
...
RegisterNetEvent('esx_jailer:unjailEscape')
AddEventHandler('esx_jailer:unjailEscape', function()
	JailTime = 0
...
RegisterNetEvent('esx_jailer:escapeNotify')
AddEventHandler('esx_jailer:escapeNotify', function(title, message)
		TriggerEvent('chat:addMessage1', "^*"..title, {61, 59, 59}, message, "fas fa-bell")
end)
